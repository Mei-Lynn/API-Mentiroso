...
public class MentirosoClient {
    ...
    private static void verJugadaAnterior() {
        try {
            ...
            String url = BASE_URL + "/jugadaAnterior?gameID=" + currentSalaID + "&username=" + myUsername;
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).GET().build();
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
    ...
    private static void subirMano() {
        try {
            ...
            String url = String.format("%s/subirMano?gameID=%s&username=%s&play=%s&n1=%s&n2=%s",
